...
-- 幂等：若 param_code 已存在（含手工在智控台创建）则跳过；id 用 max(id)+1，避免与已有主键 503 等冲突
INSERT INTO `sys_params` (id, param_code, param_value, value_type, param_type, remark)
SELECT (SELECT IFNULL(MAX(m.id), 0) + 1 FROM `sys_params` m),
       'server.agent_companion_growth_prompt_template',
...
WHERE NOT EXISTS (
    SELECT 1 FROM `sys_params` s WHERE s.param_code = 'server.agent_companion_growth_prompt_template'
);
