-- 心绪图谱：模板发版 + 儿童证据/状态/通知（与 gp_* 平行）

CREATE TABLE IF NOT EXISTS mp_template_release (
    ...
    rules_json JSON NULL COMMENT '观察/视觉/通知规则',
    status VARCHAR(16) NOT NULL DEFAULT 'draft' COMMENT 'draft|published|archived',
    published_at DATETIME NULL,
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    KEY idx_mp_release_age_status (age_band, status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='心绪图谱模板发版';

CREATE TABLE IF NOT EXISTS mp_template_node (
    id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    ...
    KEY idx_mp_node_release (release_id),
    UNIQUE KEY uk_mp_node_release_code (release_id, code)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS mp_template_edge (
    id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    release_id BIGINT NOT NULL,
    from_code VARCHAR(128) NOT NULL,
    to_code VARCHAR(128) NOT NULL,
    edge_type VARCHAR(32) NOT NULL DEFAULT 'CONTAINS',
    KEY idx_mp_edge_release (release_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS learner_mind_evidence (
    id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    ...
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    KEY idx_lme_child_release (child_id, release_id),
    KEY idx_lme_node (node_code, create_time)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS learner_mind_state (
    id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    child_id BIGINT NOT NULL,
    release_id BIGINT NOT NULL,
    node_code VARCHAR(128) NOT NULL,
    ...
    UNIQUE KEY uk_lms_child_node (child_id, node_code),
    KEY idx_lms_child_release (child_id, release_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS mp_parent_notification (
    id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    ...
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    KEY idx_mpn_parent_child (parent_user_id, child_id, is_read),
    KEY idx_mpn_child_week (child_id, create_time)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='心绪图谱家长通知';

CREATE TABLE IF NOT EXISTS mp_parent_settings (
    id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    parent_user_id BIGINT NOT NULL,
    child_id BIGINT NOT NULL,
    ...
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY uk_mps_parent_child (parent_user_id, child_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
