...
| 生效 | 提交后 **`enabled` 立即启用**，直接参与风险扫描（无需运营审核） |
...
| 类型 | 上限 | 审核通过后 |
|------|------|------------|
| `EVALUATOR` 领域判别 | 2 条 | 创建即 `enabled`，智伴合并拉取 |
| `KEYWORD` 家庭观察词 | 5 条 | 创建即写入 `child_risk_rule`（`rule_scope=PARENT`） |
...
全局儿童风险链路是否在跑，由服务端/智控台 `server.child_risk_config` 控制；入口可始终展示，提交成功后提示「已生效」。
...
> 已保存并生效，将参与孩子对话风险扫描；若触发仍会按现有方式通知您。
