...
### 3.3 查询设备全部声纹列表（主孩子 + 后台）

返回该设备在当前智能体下的**全部声纹**：主孩子声纹（若有）+ 后台录入声纹。用于小程序「声纹管理」卡片展示「已录入N人」及详情页列表。
...
**响应**：`200`，`Result<List<ParentDeviceVoicePrintVO>>`，示例：
...
| 字段 | 类型 | 说明 |
|------|------|------|
| id | string | 声纹 ID，删除时使用 |
| audioId | string | 音频文件 ID |
| sourceName | string | 声纹来源姓名 |
| introduce | string | 描述 |
| createDate | string | 创建时间 |
| canManage | boolean | 是否可管理：`true` 为主孩子声纹（家长可编辑/删除），`false` 为后台录入（仅可查看） |

**小程序用法**：卡片展示「已录入N人」即 `data.length`；详情页遍历 `data`，根据 `canManage` 决定是否显示「重新录入」「删除」按钮。
...
### 3.5 声纹管理 UI 设计说明（小程序）

依据现有声纹逻辑（主孩子 + 后台），建议如下：

**1. 卡片展示（设备详情/设置页）**

- 图标：麦克风等声纹相关图标
- 标题：声纹管理
- 副标题：**已录入 N 人**（N = `GET /parent-api/device/child/voiceprint?deviceId=xxx` 返回的 `data.length`）
- 点击进入声纹管理详情页

**2. 详情页列表**

- 调用同一接口获取 `data`，逐条渲染
- 每条展示：`sourceName`、`introduce`（若有）、`createDate`
- **主孩子声纹**（`canManage === true`）：显示「重新录入」「删除」按钮；点击删除调用 `DELETE ...?voicePrintId=xxx`
- **后台声纹**（`canManage === false`）：仅展示信息，不显示编辑/删除按钮，可加「后台录入」标签

**3. 添加主孩子声纹**

- 需先有设备主孩子（2.1）；无主孩子时引导用户先添加
- 流程：上传音频（3.1）→ 保存声纹（3.2）
- 一孩一声纹，重复保存会覆盖
