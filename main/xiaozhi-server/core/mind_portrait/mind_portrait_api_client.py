# -*- coding: utf-8 -*-
"""manager-api 心绪图谱：会话级 transcript 缓冲，连接结束时 batch 上报（LLM 判别在服务端）。"""
...
def growth_enabled(conn: Any) -> bool:
    ...
    block = cfg.get("mind_portrait") or {}
...
def _ensure_session(conn: Any) -> None:
    if getattr(conn, "mind_portrait_session_ref", None) is None:
        device_id = getattr(conn, "device_id", None) or "unknown"
        conn.mind_portrait_session_ref = f"{device_id}:{uuid.uuid4().hex[:12]}"
    if getattr(conn, "mind_portrait_turns", None) is None:
        conn.mind_portrait_turns = []
...
def _append_turn(conn: Any, role: str, text: str) -> None:
    ...
    turns: List[Dict[str, str]] = conn.mind_portrait_turns
...
def append_assistant_turn(conn: Any, text: str) -> None:
    ...
    try:
        turns = getattr(conn, "mind_portrait_turns", None)
        if not turns:
            return
...
def _post_session(payload: dict) -> None:
    ...
    try:
        ...
        with httpx.Client(
            ...
            r = client.post("config/mind-portrait/evidence/session", json=payload)
            r.raise_for_status()
...
def flush_session(conn: Any) -> None:
    ...
    try:
        turns: List[Dict[str, str]] = getattr(conn, "mind_portrait_turns", None) or []
        ...
        if user_count < _MIN_USER_TURNS:
            return
        source_ref = getattr(conn, "mind_portrait_session_ref", None) or f"session:{int(time.time())}"
        payload = {
        ...
        try:
            conn.mind_portrait_turns = []
            conn.mind_portrait_session_ref = None
        except Exception:
            pass
